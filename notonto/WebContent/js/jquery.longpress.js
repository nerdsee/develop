...
(function($, window, undefined) {
	...
	var pluginName = 'longPress', document = window.document, defaults = {/*
																			 * propertyName:
																			 * "value"
																			 */};
	...
	function showPopup(chars) {
		...
		if (!hideTimer)
			hideTimer = setTimeout(onHideTimer, 2000);
	...
	function activateRelativeLetter(i) {
		selectCharIndex(($('.long-press-letter').length + selectedCharIndex + i)
	...
	$.fn[pluginName] = function(options) {
		return this.each(function() {
			if (!$.data(this, 'plugin_' + pluginName)) {
				$.data(this, 'plugin_' + pluginName, new LongPress(this,
						options));
			}
		});
	};
