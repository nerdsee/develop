...
public class PinyinFormatter {
	 ...
	 */
	static public String formatHanyuPinyin(String pinyinStr, HanyuPinyinOutputFormat outputFormat) throws BadHanyuPinyinOutputFormatCombination {
	...
	/**
	 * Convert tone numbers to tone marks using Unicode <br/><br/> <b>Algorithm
	 * for determining location of tone mark</b><br/> A simple algorithm for
	 * determining the vowel on which the tone mark appears is as follows:<br/>
	 * <ol>
	 * <li>First, look for an "a" or an "e". If either vowel appears, it takes
	 * the tone mark. There are no possible pinyin syllables that contain both an
	 * "a" and an "e".
	...
	public static String convertToneNumber2ToneMark(final String pinyinStr) {
		...
		StringTokenizer st = new StringTokenizer(pinyinStr, " 12345", true);
		
		while (st.hasMoreTokens()) {
			String text = st.nextToken().trim();
			if (st.hasMoreTokens()) {
				text=text+st.nextToken().trim();
			}
			ret.append(convertToneNumber2ToneMarkSingle(text));
		}
		return ret.toString();
	}
	
	public static String convertToneNumber2ToneMarkSingle(final String pinyinStr) {
